...
const RecipeCard = (props) => {
  ...
  return (
    <div className="recipe relative text-left py-4 px-6 mb-4 border rounded border-solid border-gray-300 border-opacity-50 shadow-lg">
      <Link
        to={`/recipe/${id}`}
        className="recipe-link flex flex-col justify-between items center"
      >
        <img
          className="recipe-image mx-auto h-32 md:h-40 lg:h-42 w-full rounded-t mb-2 object-cover object-center leading-snug"
          src={src}
          alt={title}
        />
        <h3 className="recipe-title h-12 lg:h-14 hover:text-green-400">
          <span>{title}</span>
        </h3>
      </Link>
      ...
      {showForm === true ? (
        ...
        <TrashIcon
          className="delete-recipe absolute bottom-2 right-2 cursor-pointer w-5 h-5 lg:w-6 lg:h-6 hover:scale-110 transform transition-transform"
